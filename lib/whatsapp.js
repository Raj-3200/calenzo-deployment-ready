...
export function confirmationContentVariables({ clinic, patient, appointment, service }) {
  const ticketLink = appointment?.id ? absoluteAppUrl(`/ticket/${appointment.id}`) : ''
  const queueLink = appointment?.id ? absoluteAppUrl(`/queue/${appointment.id}`) : ''

  return {
    1: patient.fullName,
    2: clinic.name,
    3: clinic.doctorName,
    4: service?.title || 'Consultation',
    5: formatDate(appointment.appointmentDate),
    6: formatTime(appointment.appointmentTime),
    7: `#${appointment.tokenNumber}`,
    8: `${formatTime(appointment.arrivalWindowStart)} - ${formatTime(appointment.arrivalWindowEnd)}`,
    9: ticketLink,
    10: queueLink,
  }
}

export async function sendWhatsAppMessage({ to, message, contentSid = process.env.TWILIO_WHATSAPP_CONTENT_SID, contentVariables = null }) {
  ...
  if (contentSid) {
    ...
    if (contentVariables) body.set('ContentVariables', JSON.stringify(contentVariables))
  } else {
